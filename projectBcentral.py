...
# -*- coding: utf-8 -*-
"""
Created on Mon May 2 13:27:35 2022
...
--------------------------------------------------------------------------------------------
For 5 Nodes, 3426 Iterations, and with Under-Relaxing

UR = 0.999999, a = 0.999, aL = 0.999, aR = 0.00001, SU = 0.999, SUL = 0.9999999999, SUR = 0.999, q = 0.9999
...
test_converge = True                                # break the loop if error reaches a low enough amount
under_relax = True                                   # must adjust the underrelaxation constants if True

# UNDER RELAXATION CONSTANTS
UR = 0.999999                                            # for V and P 
a = 0.999                                             # for aP
aL = 0.999                                            # for aP_right
aR = 0.00001                                            # for aP_left
SU = 0.999                                            # for Su
SUL = 0.9999999999                                          # for Su_left
SUR = 0.999                                           # for Su_right
q = 0.9999                                            # for d
...
def main():
    ...
    for i in range(NUM_ITER):
        ...
        if test_converge:
            if len(vel_error) > 2:
                if abs(vel_error[-1]) < 0.01 or (abs(vel_error[-1] - vel_error[-2])) < 0.0001:
                    print("converged?")
                    print(f"Iteration: {i}")
                    break
